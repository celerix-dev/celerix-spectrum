...
export class SpectrumEngine extends EventTarget {
    ...
    private loadState(defaults?: SpectrumConfig['defaults']): ThemeState {
        ...
        let stateObject:ThemeState = {mode: "auto", isLinked: false, dark:{hue: 260, chroma:0.18}, light:{hue: 260, chroma:0.18}};
    ...
    public updateValue(theme: 'light' | 'dark', key: 'hue' | 'chroma', value: number): void {
    ...
    private applyToCSS(theme: 'light' | 'dark', key: 'hue' | 'chroma', value: number): void {
        const cssVar = `--cx-${key}-${theme}`;
        document.documentElement.style.setProperty(cssVar, value.toString());
    }

    private init(): void {
        this.setThemeMode(this.state.mode);
        this.applyToCSS('light', 'hue', this.state.light.hue);
        this.applyToCSS('light', 'chroma', this.state.light.chroma);
        this.applyToCSS('dark', 'hue', this.state.dark.hue);
        this.applyToCSS('dark', 'chroma', this.state.dark.chroma);
    }
}